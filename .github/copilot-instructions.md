...
- Releases go through `.github/workflows/release.yml`, which creates a
  ...
  `feature_flags/__init__.py`, and `uv.lock`, then enables squash auto-merge
  with branch deletion for the release PR before creating the tag and GitHub
  Release after merge
...
- **Create worktrees for new work**: create branch worktrees under the shared
  `../../worktrees/` directory (full path:
  `/home/bjorn/workspace/web_projects/worktrees/`).
